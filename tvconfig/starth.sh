...
nohup ./geth --identity tengo --datadir=./data --syncmode 'full' --verbosity 5 --port 30305 --http --http.addr=0.0.0.0 --http --http.corsdomain '*' --http.vhosts '*' --http.api 'web3,eth,net,debug,trace,txpool' --http.port 8548 --ws --ws.addr '0.0.0.0' --ws.port 8549 --ws.api 'web3,eth,net,debug,trace'  --ws.origins '*' --allow-insecure-unlock --gcmode archive --snapshot  >rpc.log 2>&1 &
